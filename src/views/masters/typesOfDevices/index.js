...
const ACTION_CONSTANT = [
  {
    ...
    show: () => hasPermisson(MODULE.TYPESOFDEVICES, ACCESS.WRITE),
  },
  {
    label: <AiFillDelete style={{ fontSize: '1.2rem' }} />,
    key: TABLE_ACTION_KEYS.DELETE,
    toolTip: 'Delete',
    show: () => hasPermisson(MODULE.TYPESOFDEVICES, ACCESS.DELETE),
  },
];

const BUTTON_CONSTANT = hasPermisson(MODULE.TYPESOFDEVICES, ACCESS.WRITE)
  ? [
      {
  ...
  return (
    <>
      ...
      <AdaptableCard className="h-full" bodyClass="h-full">
        <DataTable
          columns={newColumn(columns, MODULE.TYPESOFDEVICES, 1)}
          isCursor={false}
          data={deviceType}
          loading={loading}
